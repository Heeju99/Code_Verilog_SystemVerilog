...
module tb_AXI4_lite();
    ...
    initial begin
        ...
        @(posedge ACLK);
        #1; AWADDR = 0; AWVALID = 1; WDATA = 8'hff; WVALID = 1; BREADY = 1;//8'hff 모두 다 출력
        wait(WVALID && WREADY);
        @(posedge ACLK);
        AWVALID = 0; WVALID = 0;
        wait(BVALID == 1);
       ...
       //read
        @(posedge ACLK);
        #1; AWADDR = 0; AWVALID = 1; WDATA = 8'h00; WVALID = 1; BREADY = 1;//8'h00 GPIO를 입력 모드
        wait(WVALID && WREADY);
        @(posedge ACLK);
        AWVALID = 0; WVALID = 0;
        wait(BVALID == 1);
        ...
        io_port_mode  = 8'h00;
        io_port_input = 8'h12; //외부에서 들어오는 12 값
