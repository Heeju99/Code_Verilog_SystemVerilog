        ...
        forever begin
            ...
            if(fnd_tr.PWRITE) begin //write mode
                ...
                if(refFndReg[0] == 0) begin
                    if(fnd_tr.fndComm == 4'hf) $display("FND Enable PASS");
                    else $display("FND Enable FAIL");
                end
                else begin
                    if(refFndReg[1][3:0] == ~fnd_tr.fndComm[3:0]) 
                    $display("FND COM PASS, %h, %h", refFndReg[1][3:0], ~fnd_tr.fndComm[3:0]);
                    else 
                    $display("FND COM FAIL, %h, %h", refFndReg[1][3:0], ~fnd_tr.fndComm[3:0]);
                end
