        ...
        forever begin
            ...
            if (fnd_tr.PWRITE) begin // write mode
                refFndReg[fnd_tr.PADDR[3:2]] = fnd_tr.PWDATA;
                    ...
                    digit[3] = refFndReg[1] /1000 % 10;
                // fpr check
                    if (refFndReg[2][fnd_tr.digit_sel] == ~fnd_tr.fndFont[7])
                        $display("FND DP PASS %h, %h", refFndReg[2][fnd_tr.digit_sel], ~fnd_tr.fndFont[7]);
                    else
                        $display("FND DP FAIL %h, %h", refFndReg[2][fnd_tr.digit_sel], ~fnd_tr.fndFont[7]);
